...
if('ProxQP' in SOLVERS):
    proxqp_iter_samples   = []  
    proxqp_time_samples   = []
    proxqp_solved_samples = []

# loop on many problems
for pb_id in range(N_samples):
    print("Problem CLQR n°"+str(pb_id)+"/"+str(N_samples))
    ...
    if("ProxQP" in SOLVERS):
        print("ProxQP")
        solverproxqp = CSQP(problem, "ProxQP")
        solverproxqp.termination_tolerance = TOL
        solverproxqp.max_qp_iters = MAX_QP_ITER
        solverproxqp.eps_abs = EPS_ABS
        solverproxqp.eps_rel = EPS_REL
        ...
        if(not solved): 
            print("      FAILED !!!!")
            proxqp_iter_samples.append(MAX_QP_ITER)
            proxqp_time_samples.append(MAX_QP_TIME)
        else:
            proxqp_iter_samples.append(solverproxqp.qp_iters)
            proxqp_time_samples.append(solverproxqp.qp_time*1e3)
        print("     QP Time = ", solverproxqp.qp_time)
        print("     QP Iter = ", solverproxqp.qp_iters)
...
if('ProxQP' in SOLVERS):  
    proxqp_iter_solved = np.zeros((MAX_QP_ITER, 1))
    proxqp_time_solved = np.zeros((MAX_QP_TIME, 1))
...
for i in range(N_samples):
    ...
    if('ProxQP' in SOLVERS): 
        proxqp_iter_ik = np.array(proxqp_iter_samples)[i]
        proxqp_time_ik = np.array(proxqp_time_samples)[i]
    ...
    for j in range(MAX_QP_ITER):
        ...
        if('ProxQP' in SOLVERS): 
            if(proxqp_iter_ik < j): proxqp_iter_solved[j] += 1
    # Solve time
    for j in range(MAX_QP_TIME):
        ...
        if('ProxQP' in SOLVERS): 
            if(proxqp_time_ik < j): proxqp_time_solved[j] += 1
...
if('ProxQP' in SOLVERS): 
    ax0.plot(xdata, proxqp_iter_solved[:,0]/N_samples, color='c', linestyle='dashdot', linewidth=4, label='ProxQP') #marker='o', markerfacecolor='b', linestyle='-', markersize=12, markeredgecolor='k', alpha=1., label='SQP')
# Set axis and stuff
...
if('ProxQP' in SOLVERS): 
    ax0.plot(xdata, proxqp_time_solved[:,0]/N_samples, color='c', linestyle='dashdot', linewidth=4, label='ProxQP') #marker='o', markerfacecolor='b', linestyle='-', markersize=12, markeredgecolor='k', alpha=1., label='SQP')
# Set axis and stuff
ax0.set_ylabel('Percentage of problems solved', fontsize=26)
