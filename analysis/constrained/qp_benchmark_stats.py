...
for problem in PROBLEMS:
    ...
    for solver in SOLVERS:
        ...
        mean_iter = np.mean(iter_samples)
        std_iter = np.std(iter_samples)
        ...
        printout_lines.append(f"  Mean time : {mean_time:.4f}")
        printout_lines.append(f"  std time : {std_time:.4f}")
        ...
        printout_lines.append(f"  Mean iter : {mean_iter:.4f}")
        printout_lines.append(f"  std iter : {std_iter:.4f}")
