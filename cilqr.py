...
class CILQR(SolverAbstract):
    ...
    def solve(self, init_xs=None, init_us=None, maxiter=100, isFeasible=False, regInit=None):
        ...
        for i in range(maxiter):
            ...
            print("iter", i, "Total cost", self.cost, "gap norms", self.gap_norm, "dx norm", self.x_grad_norm, "du norm", self.u_grad_norm)
    ...
    def allocateData(self):
        ...
        self.lxmin[-1] = -np.array([np.inf, np.inf, np.inf, np.inf, np.inf, np.inf , np.inf] + [0.001]*7) 
        self.lxmax[-1] = np.array([np.inf, np.inf, np.inf, np.inf, np.inf, np.inf , np.inf] + [0.001]*7) 


        self.lumin = [-1*clip_ctrl for m  in self.problem.runningModels] 
        self.lumax = [clip_ctrl for m  in self.problem.runningModels] 
        #
