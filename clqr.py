...
class CLQR(SolverAbstract, QPSolvers):
    ...
    def update_lagrangian_parameters(self):
        ...
        self.norm_primal_rel, self.norm_dual_rel, = -np.inf, -np.inf
        ...
        for t, (model, data) in enumerate(zip(self.problem.runningModels, self.problem.runningDatas)):
            ...
            self.norm_primal_rel = max(self.norm_primal_rel, max(abs(self.Cx[t]@self.dx[t])), max(abs(self.Cu[t]@self.du[t])),\
                                         max(abs(self.xz[t])), max(abs(self.uz[t])))
            self.norm_dual_rel = max(self.norm_dual_rel, max(abs(data.Lxx@self.dx[t])), max(abs(data.Luu@self.du[t])),\
                                max(abs(self.Cx[t].T@self.xy[t])), max(abs(self.Cu[t].T@self.uy[t])), max(abs(data.Lx)), max(abs(data.Lu)))
        ...
        self.norm_dual_rel = max(self.norm_dual_rel, max(abs(data.Lxx@self.dx[t])),\
                            max(abs(self.Cx[t].T@self.xy[t])), max(abs(data.Lx)))

        # print(self.norm_primal, self.norm_dual )
