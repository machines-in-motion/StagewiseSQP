...
def solveOCP(q, v, ddp, nb_iter, node_id_circle, target_reach, TASK_PHASE):
...
class KukaCircleFADMM:
    ...
    def warmup(self, thread):
        self.nb_iter = 100        
        logger.warning(self.ddp.xs)
        logger.warning([self.x0 for i in range(self.config['N_h']+1)])
        ...
        # USE pipe
        if(USE_PIPE):
            self.subp = Process(target=rt_SolveOCP, args=(self.child_conn, self.ddp, self.nb_iter, self.node_id_circle, self.target_position, self.TASK_PHASE))
            self.subp.start()
