...
from controllers.kuka_reach_SQP import KukaReachSQP
...
ctrl = KukaReachSQP(head, pin_robot, config, run_sim=SIM, use_SQP=config['USE_SQP'])
# ctrl.warm_start(100)
...
if SIM:
    ...
    thread_head.start_logging(5, "/tmp/kuka_reach_SQP_sim_USE_SQP="+str(config['USE_SQP'])+".mds")
    thread_head.sim_run_timed(100000)
    # thread_head.stop_logging()
    thread_head.plot_timing()
else:
    thread_head.start()
    thread_head.start_logging(15, "/tmp/kuka_reach_SQP_real_USE_SQP="+str(config['USE_SQP'])+".mds")
    # time.sleep(30)
    # thread_head.plot_timing()
