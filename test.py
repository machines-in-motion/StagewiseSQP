...
# print(d_relaxed - np.array(ddp2.xtilde_k_1))

# assert np.linalg.norm( d_relaxed- np.array(ddp2.xtilde_k_1)) < 1e-6, "Test failed"

for i in range(len(ddp1.xs)):
    print(ddp1.xs[i] - ddp2.xs[i])

assert np.linalg.norm(np.array(ddp1.xs) - np.array(ddp2.xs)) < 1e-6, "Test failed"

assert np.linalg.norm(np.array(ddp1.us) - np.array(ddp2.us)) < 1e-6, "Test failed"

dx_relaxed = np.array(ddp1.dx_relaxed).flatten()[4:]
du_relaxed = np.array(ddp1.du_relaxed).flatten()
d_relaxed = np.hstack((dx_relaxed, du_relaxed))
assert np.linalg.norm( d_relaxed- np.array(ddp2.x_k_1)) < 1e-6, "Test failed"

xz = np.array(ddp1.xz).flatten()[4:]
uz = np.array(ddp1.uz).flatten()[:-2]
z = np.hstack((xz, uz))
assert np.linalg.norm(z - np.array(ddp2.z_k)) < 1e-6, "Test failed"

xy = np.array(ddp1.xy).flatten()[4:]
uy = np.array(ddp1.uy).flatten()[:-2]
y = np.hstack((xy, uy))
assert np.linalg.norm(y - np.array(ddp2.y_k)) < 1e-6, "Test failed"

rho_x = np.array(ddp1.rho_vec_x).flatten()[4:]
rho_u = np.array(ddp1.rho_vec_u).flatten()[:-2]
rho = np.hstack((rho_x, rho_u))

assert np.linalg.norm(rho - np.array(ddp2.rho_vec_boyd)) < 1e-6, "Test failed"

assert np.linalg.norm(ddp1.rho_estimate_sparse - ddp2.rho_estimate_boyd) < 1e-6, "Test failed"
